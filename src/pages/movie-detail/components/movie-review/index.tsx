...
const MovieReview = forwardRef<HTMLDivElement, ReviewProps>(({ review, movieId, noBorder }, ref) => {
  ...
  // 리뷰가 없으면 null 반환
  if (!currentReview) {
    console.warn("MovieReview: 리뷰 데이터가 없습니다.");
    return null;
  }
  ...
  const handleInteraction = async (type: "like" | "dislike") => {
    ...
    if (currentReview.isAuthor) {
      showToast("본인이 작성한 한줄평에는 좋아요/싫어요를 할 수 없습니다.", "none");
      return;
    }
    ...
    try {
      await toggleLineReviewLike(currentReview.id, isLike ? "LIKE" : "DISLIKE");
      ...
      <TotalReviewsContainer>
        <ReviewBody ref={ref} noBorder={noBorder}>
          ...
          <ReviewDetailsContainer>
            ...
            <PointSvg />
            <ReviewDetailsText>{formatToKST(currentReview.createdAt)}</ReviewDetailsText>
            <PointSvg />
            <ReviewDetailsText>신고</ReviewDetailsText>
          </ReviewDetailsContainer>
          <ThumbsButtonWrapper>
            <ThumbsButton onClick={() => handleInteraction("like")} active={currentReview.isLiked}>
              {currentReview.isLiked ? <ThumbsUpActiveSvg /> : <ThumbsUpSvg />}
              {currentReview.likes}
            </ThumbsButton>
            <ThumbsButton
            ...
            >
              {currentReview.isDisliked ? <ThumbsDownActiveSvg /> : <ThumbsDownSvg />}
