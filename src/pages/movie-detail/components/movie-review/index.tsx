...
const MovieReview = ({ reviews, lastReviewRef }: ReviewProps) => {
  ...
  // 좋아요 및 싫어요 처리 함수
  const handleInteraction = (index: number, type: "like" | "dislike") => {
    setReviewInteractions((prev) =>
      prev.map((interaction, idx) => {
        if (idx === index) {
          ...
          return {
            ...interaction,
            liked: isLike ? !interaction.liked : false,
            disliked: !isLike ? !interaction.disliked : false,
            likes: isLike
