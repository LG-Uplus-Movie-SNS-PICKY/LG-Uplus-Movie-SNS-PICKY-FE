...
const reviewHandler = [
    // 특정 영화의 댓글을 추가
    http.post(`${import.meta.env.VITE_SERVER_URL}/api/v1/linereview/create`, (_a) => __awaiter(void 0, [_a], void 0, function* ({ request }) {
        ...
        return HttpResponse.json({ message: "Resource created successfully" }, { status: 201, statusText: "Created" });
    })),
    http.patch(`${import.meta.env.VITE_SERVER_URL}/api/v1/linereview/:lineReviewId`, (_a) => __awaiter(void 0, [_a], void 0, function* ({ params, request }) {
        ...
        if (!lineReviewId || Number.isNaN(Number(lineReviewId)))
            return HttpResponse.json({ message: "Invalid or missing Line Review Id" }, { status: 400, statusText: "LINE_REVIEW_ID_MISSING" });
        ...
        return HttpResponse.json({ message: "Comment updated successfully" }, { status: 200, statusText: "OK" });
    })),
    // 특정 영화에 대한 한줄평 목록을 조회
    http.get(`${import.meta.env.VITE_SERVER_URL}/api/v1/linereview/movie/:movieId`, ({ params, request }) => {
        // params 없거나
        const { movieId } = params;
        // Movie Id를 제대로 가지고 오지 못한 경우 + 숫자가 아닌 경우
        if (!movieId || Number.isNaN(Number(movieId)))
            return HttpResponse.json({ message: "Invalid or missing movieId" }, { status: 400, statusText: "MOVIE_ID_MISSING" });
        // JSON으로 구성된 파일 중 Movie Id에 맞는 값만 필터 시킨다.
        const review = response.filter((data) => data.movieId === Number(movieId));
