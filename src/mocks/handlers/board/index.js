...
const boardHandlers = [
    ...
    http.post(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:boardId`, () => { }),
    // 무비로그 삭제 API(Mocking Object)
    http.delete(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:boardId`, () => { }),
    // 무비로그 조회 API(Mocking Object) - 무비로그 탭 최신순 목록 조회
    http.get(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/all`, ({ request }) => {
        var _a;
        const authorization = (_a = request === null || request === void 0 ? void 0 : request.headers) === null || _a === void 0 ? void 0 : _a.get("Authorization");
        // 권한이 없을 경우 403에러 발생
        if (!authorization) {
    ...
    http.get(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:movieId`, ({ params, request }) => {
        var _a;
        const authorization = (_a = request === null || request === void 0 ? void 0 : request.headers) === null || _a === void 0 ? void 0 : _a.get("Authorization");
        ...
        return HttpResponse.json({ data: resposne.filter((data) => data.movie.id === Number(movieId)) }, { status: 200 });
    }),
    // 무비로그 좋아요 API(Mocking Object) - 기능을 잘 모르겠음
    // 특정 게시물 댓글 생성 API(Mocking Object)
    http.post(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:boardId/coments`, () => { }),
    // 특정 게시물 댓글 조회 API(Mocking Object)
    http.get(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:boardId/coments`, () => { }),
    // 특정 댓글 삭제 API(Mocking Object)
    http.delete(`${import.meta.env.VITE_SERVER_URL}/api/v1/board/:boardId/coments`, () => { }),
];
